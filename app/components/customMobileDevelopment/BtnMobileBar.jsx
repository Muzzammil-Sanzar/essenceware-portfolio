...
export default function BtnMobileBar() {
  const [isBooking, setIsBooking] = useState(false);

  // const handleChatMe = () => {
  //   if (window.Tawk_API && window.innerWidth <= 768) {
  //     window.Tawk_API.maximize();
  //   }
  // };
  return (
    <>
      <div className="pt-7 md:py-10 xl:hidden">
        <div className="flex items-center justify-center gap-4">
          <div
            onClick={e => setIsBooking(true)}
            className="font-semibold md:w-[193px] text-sm whitespace-nowrap md:text-base cursor-pointer flex items-center justify-center gap-2 bg-blue-150 text-white rounded-lg px-6 md:px-4 py-[9px] md:py-[13px]"
          >
            {/* <svg
              width="9"
              height="9"
          ...
          {/* <div
            onClick={(e) => setIsBooking(true)}
            className="font-semibold  bg-blue-150 text-white text-sm whitespace-nowrap md:text-base flex cursor-pointer items-center gap-2 rounded-lg px-4 md:px-4 py-[9px] md:py-[13px]"
          >
            <svg
      ...
      <RequestPricingModal isOpen={isBooking} setIsOpen={setIsBooking} />
    </>
  );
}
