...
import industryData from "./IndustrySectionData.json";
import { useEffect, useState } from "react";
import Link from "next/link";

export default function IndustriesSection({ link }) {
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-6 gap-y-9">
        {industryData &&
          industryData.slice(0, visibleItems).map((industry, index) => (
            <Link key={index} href={link ? `${link}/${industry.key}` : `/Mobile-App-Development/${industry.key}`}>
